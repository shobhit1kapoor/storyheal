...
for SERVICE in $SERVICES; do
  ...
  case "$SERVICE" in
    ...
    storyheal-cli|storyheal-widget-cli)
      cd "$REPO_ROOT/$SERVICE_DIR"
      if npm run build; then
        echo "  ✓ $SERVICE passed"
      else
    ...
    storyheal-ai|storyheal-api|storyheal-rag|storyheal-platform|storyheal-workflow|storyheal-plugin-runtime|storyheal-device-control)
      cd "$REPO_ROOT/$SERVICE_DIR"
      if [ -f "pyproject.toml" ]; then
        ...
        if poetry run flake8 "$SRC_DIR" 2>/dev/null; then
          ...
          # Try ruff as fallback (storyheal-platform uses ruff)
