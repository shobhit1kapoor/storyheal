...
async def enable_platform(
    ...
    # Notify storyheal-platform to start Slack Socket Mode handler
...
async def disable_platform(
    ...
    # Notify storyheal-platform to stop Slack Socket Mode handler
    if platform.type == "slack":
...
async def _notify_slack_platform_reload(platform: Platform) -> None:
    """Notify storyheal-platform to start Slack Socket Mode handler for a platform."""
    platform_base_url = settings.PLATFORM_BASE_URL if hasattr(settings, 'PLATFORM_BASE_URL') else "http://storyheal-platform:8003"
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if response.status_code == 200:
                ...
                logger.warning("Failed to notify storyheal-platform for Slack reload: %s", response.text)
    except Exception as e:
        logger.warning("Error notifying storyheal-platform for Slack reload: %s", str(e))


async def _notify_slack_platform_stop(platform_id: str) -> None:
    """Notify storyheal-platform to stop Slack Socket Mode handler for a platform."""
    platform_base_url = settings.PLATFORM_BASE_URL if hasattr(settings, 'PLATFORM_BASE_URL') else "http://storyheal-platform:8003"
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if response.status_code == 200:
                ...
                logger.warning("Failed to notify storyheal-platform for Slack stop: %s", response.text)
    except Exception as e:
        logger.warning("Error notifying storyheal-platform for Slack stop: %s", str(e))
...
async def platform_callback_forward(
    ...
    """Forward third-party platform callbacks to the StoryHeal Platform Service.
