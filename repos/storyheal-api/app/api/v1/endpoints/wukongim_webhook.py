...
from app.models import Platform, Staff, Visitor, ChannelMember, VisitorSession, SessionStatus
from app.services.knowledge_ops_service import capture_evidence
...
async def _handle_msg_notify_batch(messages: Any, db: Session) -> None:
    ...
    evidence_messages: Dict[UUID, List[str]] = {}
            ...
            if isinstance(raw_payload, str):
                try:
                    ...
                    try:
                        ...
                        decoded = {"content": raw_payload}
            if isinstance(decoded, dict):
                text_value = decoded.get("content") or decoded.get("text") or decoded.get("message")
                if isinstance(text_value, str) and text_value.strip():
                    evidence_messages.setdefault(visitor_id, []).append(text_value)
    ...
    for visitor_id, excerpts in evidence_messages.items():
        visitor = db.query(Visitor).filter(
            Visitor.id == visitor_id, Visitor.deleted_at.is_(None)
        ).first()
        if not visitor:
            continue
        ...
        for excerpt in excerpts:
            capture_evidence(
                ...
                text=excerpt,
                source_uri=f"wukongim://visitor/{visitor_id}",
            )
        db.commit()
