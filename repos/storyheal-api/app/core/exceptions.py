...
class StoryHealAPIException(Exception):
    """Base exception for StoryHeal API."""
...
class ValidationError(StoryHealAPIException):
...
class NotFoundError(StoryHealAPIException):
...
class ConflictError(StoryHealAPIException):
...
class AuthenticationError(StoryHealAPIException):
...
class AuthorizationError(StoryHealAPIException):
...
class RateLimitError(StoryHealAPIException):
...
class ExternalServiceError(StoryHealAPIException):
...
async def storyheal_api_exception_handler(request: Request, exc: StoryHealAPIException) -> JSONResponse:
    """Handle StoryHeal API exceptions."""
    request_id = str(uuid4())
    
    logger.error(
        f"StoryHeal API Exception: {exc.code} - {exc.message}",
        extra={
