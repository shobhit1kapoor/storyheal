...
class Settings(BaseSettings):
    ...
    API_BASE_URL: str = Field(
        ...
        description="Public-facing base URL for this StoryHeal API service (used to construct callback URLs)"
    )
    ...
    RAG_INTERNAL_API_KEY: str = Field(
        default="change-me-in-production",
        description="Shared key for authenticated API-to-RAG indexing operations",
    )
    ...
    # Platform Service settings (StoryHeal Platform Service)
    PLATFORM_SERVICE_URL: str = Field(
        default="http://localhost:8003",
        description="URL of the StoryHeal Platform Service",
    )
    ...
    # StoryHeal knowledge operations
    CELERY_BROKER_URL: str = Field(
        default="redis://localhost:6379/4",
        description="Durable queue for knowledge analysis and Storyblok webhook processing",
    )
    CELERY_RESULT_BACKEND: str = Field(
        default="redis://localhost:6379/4",
        description="Result backend for knowledge operations",
    )
    KNOWLEDGE_MIN_CONFIDENCE: float = Field(default=0.75, ge=0, le=1)
    KNOWLEDGE_MIN_OCCURRENCES: int = Field(default=2, ge=1)
    KNOWLEDGE_MIN_EVIDENCE_SCORE: float = Field(default=0.80, ge=0, le=1)
    KNOWLEDGE_MIN_QUALITY_SCORE: float = Field(default=85.0, ge=0, le=100)
    KNOWLEDGE_EVIDENCE_RETENTION_DAYS: int = Field(default=30, ge=1, le=365)
    ...
    PLUGIN_RUNTIME_URL: str = Field(
        ...
        description="URL of the storyheal-plugin-runtime service",
    )
    ...
    DEVICE_CONTROL_SERVICE_URL: str = Field(
        ...
        description="URL of the storyheal-device-control service",
    )
