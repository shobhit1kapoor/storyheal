...
class AIServiceClient:
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        headers = {
            ...
            "User-Agent": "StoryHeal-API-Service/0.1.0",
        }
    ...
    async def run_knowledge_agent(
        ...
        """Run one schema-constrained StoryHeal knowledge agent."""
        response = await self._make_request(
            "POST",
            f"/api/v1/knowledge-agents/{agent_type}/run",
            json_data=payload,
            params={"project_id": project_id},
            extra_headers={"X-Internal-Key": self.api_key or ""},
        )
        result = await self._handle_response(response)
        if not isinstance(result, dict):
            raise HTTPException(status_code=502, detail="Knowledge agent returned an invalid response")
        return result


    # ------------------------------------------------------------------
    # Skill management (proxy to storyheal-ai /api/v1/skills)
    # ------------------------------------------------------------------
    ...
    async def import_skill(self, project_id: str, import_data: Dict[str, Any]) -> Dict[str, Any]:
        """Import a skill from GitHub (proxy to storyheal-ai)."""
        response = await self._make_request(
            "POST",
