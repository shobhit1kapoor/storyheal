...
async def _get_or_create_session(
    ...
    # A new conversation shortly after a resolved close is a measurable reopen.
    reopen_cutoff = datetime.utcnow() - timedelta(hours=24)
    previous = db.query(VisitorSession).filter(
        VisitorSession.visitor_id == visitor_id,
        VisitorSession.project_id == project_id,
        VisitorSession.status == SessionStatus.CLOSED.value,
        VisitorSession.closed_at.is_not(None),
        VisitorSession.closed_at >= reopen_cutoff,
    ).order_by(VisitorSession.closed_at.desc()).first()
    if previous:
        previous.reopened_within_24h = True
        previous.reopened_at = datetime.utcnow()
        db.query(ResponseMetric).filter(ResponseMetric.session_id == previous.id).update(
            {ResponseMetric.reopened_within_24h: True}, synchronize_session=False
        )
