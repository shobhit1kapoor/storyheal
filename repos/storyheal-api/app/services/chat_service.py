...
from app.core.metrics import FIRST_RESPONSE_DURATION
...
from app.models import StoryblokConnection, VisitorSession
...
async def forward_ai_event_to_wukongim(
    ...
    try:
        ...
        top_level_sources = event_data.get("sources")
        if isinstance(top_level_sources, list) and isinstance(data, dict):
            data = {**data, "sources": top_level_sources}
...
async def _canonical_storyblok_context(project_id: str, query: str) -> tuple[str, list[dict[str, Any]]]:
    """Retrieve published Storyblok vectors and format grounded context plus typed citations."""
    ...
    try:
        response = await rag_client.search_collection_documents(
            project_id,
            collection_id,
            {"query": query, "limit": 6, "min_score": 0.15, "search_mode": "hybrid"},
        )
    except Exception as exc:
        logger.warning("Canonical Storyblok retrieval failed", extra={"error": str(exc)})
        return "", []
    ...
    for result in results if isinstance(results, list) else []:
        ...
        if tags.get("source_type") != "storyblok":
            continue
        story_uuid = str(tags.get("story_uuid") or "")
        full_slug = str(tags.get("slug") or "")
        locale = str(tags.get("locale") or "en")
        key = (story_uuid, locale)
        if not story_uuid or key in seen:
            continue
        seen.add(key)
        title = str(result.get("document_title") or "Storyblok knowledge")
        preview = str(result.get("content") or result.get("content_preview") or "")
        ...
        sources.append({
            ...
            "url": str(tags.get("source_url") or f"/help/{full_slug}"),
        })
    if not context_blocks:
        return "", []
    context = (
        "\n\nUse the following published Storyblok knowledge as canonical truth. "
        "Do not rely on draft content. Cite the supplied sources in factual answers.\n\n"
        + "\n\n---\n\n".join(context_blocks)
    )
    return context, sources


def _record_first_response(session_id: Optional[str], started: float) -> None:
    if not session_id:
        return
    with SessionLocal() as db:
        session = db.query(VisitorSession).filter_by(id=session_id).first()
        if session and session.first_response_time_ms is None:
            if session.last_message_at:
                received_at = session.last_message_at
                if received_at.tzinfo is None:
                    received_at = received_at.replace(tzinfo=timezone.utc)
                elapsed_ms = int((datetime.now(timezone.utc) - received_at).total_seconds() * 1000)
                session.first_response_time_ms = max(0, elapsed_ms)
            else:
                session.first_response_time_ms = int((time.perf_counter() - started) * 1000)
            FIRST_RESPONSE_DURATION.observe(session.first_response_time_ms / 1000)
            db.commit()
...
async def process_ai_stream_to_wukongim(
    ...
    canonical_context, sources = await _canonical_storyblok_context(project_id, message)
    grounded_system_message = (system_message or "") + canonical_context
    ...
    try:
        async for stream_event_type, data in ai_client.run_supervisor_agent_stream(
            ...
            if event_type in {"workflow_completed", "agent_response_complete"} and isinstance(data, dict):
            ...
            if content_chunk:
                ...
                if not first_token_recorded:
                    _record_first_response(session_id, started)
...
async def handle_ai_response_non_stream(
    ...
    try:
        ...
        return {"success": True, "content": full_content, "data": last_data, "sources": sources}
    except Exception as e:
