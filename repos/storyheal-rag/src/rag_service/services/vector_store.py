...
class VectorStoreService:
        ...
        try:
            ...
            for batch_idx in range(0, len(documents), max_batch_size):
                ...
                try:
                    ...
                    # A caller must never mark content indexed when any vector
                    # batch failed. Raising also makes the external-source
                    # endpoint retry-safe and preserves an honest failed state.
                    raise
    ...
    async def delete_document_embedding_for_project(
        self, document_id: UUID, project_key: str, embeddings_client: Any
    ) -> bool:
        """Delete an embedding through the same project-scoped store used to create it."""
        try:
            vector_store = await self.get_vector_store_for_project(project_key, embeddings_client)
            import asyncio
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: vector_store.delete(ids=[str(document_id)])
            )
            return True
        except Exception as exc:
            logger.error(f"Failed to delete project document embedding {document_id}: {exc}")
            return False
