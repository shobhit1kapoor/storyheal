...
from app.exceptions import StoryHealAIServiceException
...
class ProviderNotFoundError(StoryHealAIServiceException):
...
class ProviderNotActiveError(StoryHealAIServiceException):
...
class UnsupportedProviderError(StoryHealAIServiceException):
...
class ChatCompletionError(StoryHealAIServiceException):
...
class ChatService:
    ...
    def _create_openai_client(self, provider: LLMProvider) -> AsyncOpenAI:
        ...
        if (provider.vendor or "").lower() == "ollama":
            # A cold 8B model on CPU can spend more than a minute loading and
            # evaluating the prompt. Cancelling at the generic cloud timeout
            # forces Ollama to restart that work on every retry.
