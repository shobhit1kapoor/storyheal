...
class PluginToolCreate(BaseModel):
    ...
    name: str = Field(..., description="Tool name (e.g., 'plugin:com.storyheal.ticket:create_ticket')")
    ...
    endpoint: Optional[str] = Field(None, description="Plugin endpoint (e.g., 'plugin://com.storyheal.ticket')")
...
async def register_plugin_tool(
    ...
    Called by storyheal-plugin-runtime when a plugin connects and registers MCP tools.
    """
...
async def delete_plugin_tools_by_prefix(
    prefix: str = Query(..., description="Tool name prefix (e.g., 'plugin:com.storyheal.ticket:')"),
    db: AsyncSession = Depends(get_db),
) -> Dict[str, Any]:
    """
    Delete all plugin tools with names starting with the given prefix.
    
    Called by storyheal-plugin-runtime when a plugin disconnects.
    """
