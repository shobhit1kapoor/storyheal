"""Seed script: Register storyheal-device-control as an MCP tool in the ai_tools table.

Usage:
    python -m scripts.seed_device_control_tool --project-id <UUID>

This creates a LOCAL MCP tool record pointing to the storyheal-device-control
...
DEVICE_CONTROL_ENDPOINT = "http://storyheal-device-control:8085/mcp/{device_id}"
    ...
    if not db_url:
        try:
            ...
            print(
                ...
                "Pass --database-url or run from the storyheal-ai directory.",
                file=sys.stderr,
            )
            sys.exit(1)
