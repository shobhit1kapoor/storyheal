...
export default function App(){
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      const d = e.data
      if (d && d.type === 'storyheal:visibility' && typeof d.open === 'boolean') {
        setVisible(d.open)
        if (d.open) {
          clearUnreadCount()
        }
  ...
  useEffect(() => {
    console.log('[App] Sending StoryHeal_WIDGET_UNREAD to parent, count:', unreadCount)
    try {
      window.parent?.postMessage({ type: 'StoryHeal_WIDGET_UNREAD', payload: { count: unreadCount } }, '*')
    } catch (e) {
      console.error('[App] Failed to send StoryHeal_WIDGET_UNREAD:', e)
    }
  }, [unreadCount])
    ...
    if (content) {
      try {
        window.parent?.postMessage({ 
          type: 'StoryHeal_SHOW_TOAST', 
          payload: { 
            title: pConfig.widget_title || 'StoryHeal',
            body: content,
  ...
  const title = useMemo(()=> pConfig?.widget_title || 'StoryHeal', [pConfig?.widget_title])
  ...
  useEffect(()=>{
    ...
    try { window.parent?.postMessage?.({ type: 'StoryHeal_WIDGET_CONFIG', payload: { theme_color: theme } }, '*') } catch {}
  }, [theme])

  // Notify host page to position the UI iframe
  useEffect(()=>{
    try { window.parent?.postMessage?.({ type: 'StoryHeal_WIDGET_CONFIG', payload: { position: pos } }, '*') } catch {}
  }, [pos])

  // Notify host page about expanded state
  useEffect(()=>{
    try { window.parent?.postMessage?.({ type: 'StoryHeal_WIDGET_CONFIG', payload: { expanded: !!isExpanded } }, '*') } catch {}
  }, [isExpanded])
  ...
  useEffect(()=>{
    ...
    const SS_STARTED_AT = 'storyheal_session_started_at'
    const SS_PAGES = 'storyheal_session_pages'
    try {
    ...
    const onMsg = (e: MessageEvent) => {
      ...
      if (d?.type === 'StoryHeal_HOST_PAGE_INFO' && d.payload?.page_url) {
      ...
      } else if (d?.type === 'StoryHeal_HOST_PAGE_EXIT') {
        flush('exit')
      ...
      } else if (d?.type === 'StoryHeal_WIDGET_SHOWN') {
        try { useChatStore.getState().clearUnreadCount() } catch {}
      } else if (d?.type === 'StoryHeal_TRACK_EVENT' && d.payload) {
        const uid = useChatStore.getState().myUid || myUid
        const apiKey = resolveApiKey()
        if (!cfg.apiBase || !uid || !apiKey) return
        const p = d.payload || {}
    ...
    if (useChatStore.getState().myUid || myUid) {
      try { window.parent?.postMessage?.({ type: 'StoryHeal_REQUEST_PAGE_INFO' }, '*') } catch {}
    }

    // We rely on host SDK to send StoryHeal_HOST_PAGE_EXIT; avoid double triggering here.
  ...
  const requestClose = () => {
    try { (window.parent as any)?.StoryHealWidget?.hide?.(); } catch {}
  };
  ...
  return (
    <ThemeProvider initialMode={themeMode}>
      <WidgetWrap role="dialog" aria-label="Customer Support">
        ...
        <a
          href="https://storyheal.example.com"
          target="_blank"
          ...
          `}
        >Powered by storyheal.example.com</a>
      </WidgetWrap>
