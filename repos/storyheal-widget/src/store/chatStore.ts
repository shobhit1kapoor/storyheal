...
import type { ChatMessage, MessagePayload, MixedPart, JSONRenderPatchPart, SourceCitation } from '../types/chat'
...
export type ChatState = {
  ...
  attachSources: (clientMsgNo: string, sources: SourceCitation[]) => void
...
function toPayloadFromAny(raw: any): MessagePayload {
  ...
  const t = raw?.type
  if (t === 1 && typeof raw?.content === 'string') return {
    type: 1,
    content: raw.content,
...
function normalizeSources(raw: unknown): SourceCitation[] {
  ...
  return raw.filter((item): item is SourceCitation => Boolean(
    item && typeof item === 'object' &&
    typeof (item as SourceCitation).story_uuid === 'string' &&
    typeof (item as SourceCitation).url === 'string'
  ))
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  initIM: async (cfg: ChatConfig) => {
    ...
    try {
      ...
      offMsg = IMService.onMessage((m) => {
        ...
        const title = pConfig.widget_title || 'StoryHeal'
        let content = ''
      ...
      offCustom = IMService.onCustom((ev: unknown) => {
        try {
          ...
          if (newEventType === 'stream.finish') {
            ...
            if (clientMsgNo) get().attachSources(clientMsgNo, normalizeSources(eventData?.payload?.sources))
          ...
          if (customEvent.type === '___TextMessageStart') {
            ...
            playNotificationSound()
            const pConfig = usePlatformStore.getState().config
            void showBrowserNotification(pConfig.widget_title || 'StoryHeal', {
  ...
  attachSources: (clientMsgNo: string, sources: SourceCitation[]) => {
    if (!clientMsgNo || !sources.length) return
    set(state => ({
      messages: state.messages.map(message => message.clientMsgNo === clientMsgNo
        ? { ...message, sources, payload: message.payload.type === 1 ? { ...message.payload, sources } : message.payload }
        : message),
    }))
  },
