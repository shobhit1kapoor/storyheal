...
import { AlertCircle, BookOpen, ExternalLink, RotateCw, ThumbsDown, ThumbsUp, Trash2 } from 'lucide-react'
...
`
const Sources = styled.div`margin-top:8px; display:flex; flex-direction:column; gap:6px; max-width:320px;`
const SourceLink = styled.a`display:flex; align-items:center; justify-content:space-between; gap:8px; padding:8px 10px; border:1px solid var(--border-color,#e5e7eb); border-radius:10px; color:var(--text-primary,#1f2937); background:var(--bg-secondary,#f8fafc); font-size:12px; text-decoration:none;`
const Feedback = styled.div`display:flex; align-items:center; gap:4px; margin-top:6px; button{border:0;background:transparent;color:var(--text-secondary,#6b7280);cursor:pointer;padding:4px;border-radius:6px} button:hover{background:var(--bg-secondary,#f3f4f6);color:var(--text-primary,#111827)}`
...
export default function MessageList({ messages }: { messages: ChatMessage[] }){
  ...
  const sendFeedback = async (message: ChatMessage, helpful: boolean) => {
    const platformApiKey = resolveApiKey()
    if (!apiBase || !platformApiKey || feedback[message.id] !== undefined) return
    const content = message.payload.type === 1 ? message.payload.content : message.id
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(content))
    const questionHash = Array.from(new Uint8Array(hash)).map(value => value.toString(16).padStart(2, '0')).join('')
    const response = await fetch(`${apiBase.replace(/\/$/, '')}/v1/knowledge-ops/public/feedback`, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        helpful,
        source_refs: message.sources || ('sources' in message.payload ? message.payload.sources : []) || [],
      }),
    })
    if (response.ok) setFeedback(current => ({ ...current, [message.id]: helpful }))
  }
    ...
    <Main ref={ref} role="log" aria-live="polite">
      <List>
        ...
        {items.map(m => {
          ...
          const sources = m.sources || (payload.type === 1 ? payload.sources : undefined) || []
          ...
          <li key={m.key}>
            ...
            {m.role === 'agent' && sources.length > 0 && (
              <Sources>
                <div style={{fontSize:11,fontWeight:600,color:'var(--text-secondary,#6b7280)',display:'flex',alignItems:'center',gap:5}}><BookOpen size={12}/> Published sources</div>
                {sources.map(source => <SourceLink key={`${source.story_uuid}:${source.locale}`} href={source.url} target="_blank" rel="noreferrer"><span>{source.title}</span><ExternalLink size={12}/></SourceLink>)}
                <Feedback aria-label="Was this answer helpful?"><button onClick={() => void sendFeedback(m, true)} title="Helpful" style={feedback[m.id] === true ? {color:'#0f766e',background:'#ccfbf1'} : undefined}><ThumbsUp size={14}/></button><button onClick={() => void sendFeedback(m, false)} title="Not helpful" style={feedback[m.id] === false ? {color:'#be123c',background:'#ffe4e6'} : undefined}><ThumbsDown size={14}/></button></Feedback>
              </Sources>
            )}
            {m.role==='user' && (
