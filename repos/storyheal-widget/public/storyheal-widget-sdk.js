(function(){
  const SDK_NAME = 'StoryHealWidget';
  const UI_IFRAME_NAME = 'storyheal-ui-frame';
  const UI_CONTAINER_ID = 'storyheal-ui-container';
  const CTRL_IFRAME_NAME = 'storyheal-controller-frame';
  const READY_TIMEOUT = 8000; // ms, fallback if UI load takes too long
  let __storyheal_lastInstance = null; // holds latest injected instance (show/hide)
  ...
  function getCurrentScript(){
    ...
    for(let i=scripts.length-1;i>=0;i--){
      ...
      if(src.includes('storyheal-widget-sdk.js') || src.includes('/widget.js') || /(?:^|[\\/])widget\.js(?:[?#]|$)/.test(src)) return s;
    }
    return null;
  }
  ...
  function createUIFrame(baseUrl){
    ...
    try {
      ...
      console.error('[StoryHeal SDK] Failed to set UI iframe src', e);
    }
  ...
  function createLauncher(){
    ...
    // default icon: StoryHeal logo image (preserve original color)
  ...
  function createControllerFrame(baseUrl, opts){
    ...
    try {
      ...
      console.error('[StoryHeal SDK] Failed to create controller iframe', e);
    }

    // Notify UI-ready once when the controller finishes its first load
    ctrl.addEventListener('load', ()=>{
      try { 
        window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'storyheal:ui-ready', name:UI_IFRAME_NAME }, '*'); 
      } catch(e) {
        console.error('[StoryHeal SDK] Failed to notify controller iframe', e);
      }
    }, { once: true });
  ...
  function inject(baseUrl, opts){
    ...
    function applyOpen(open){
      ...
      if(open){
        ...
        try { window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'StoryHeal_WIDGET_SHOWN' }, '*'); } catch(e) {}
      } else {
        // hide container
      ...
      try { window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'storyheal:visibility', open: !!open }, '*'); } catch(e) {}
    }
    ...
    const mount = () => {
      ...
      let __storyheal_pg_timer = null;
      const notifyPageInfo = ()=>{
        try {
          const payload = { page_url: window.location.href, title: document.title, referrer: document.referrer || '' };
          window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'StoryHeal_HOST_PAGE_INFO', payload }, '*');
        } catch(e) { /* noop */ }
      };
      const notifyPageInfoDebounced = ()=>{ try { if(__storyheal_pg_timer) clearTimeout(__storyheal_pg_timer); __storyheal_pg_timer = setTimeout(notifyPageInfo, 300); } catch(_){} };


      // Notify controller on page exit (before unload/hidden)
      let __storyheal_last_exit_at = 0;
      const notifyPageExit = ()=>{
        try {
          const now = Date.now();
          if (now - __storyheal_last_exit_at < 500) return; // throttle duplicate events
          __storyheal_last_exit_at = now;
          const payload = { page_url: window.location.href, title: document.title, referrer: document.referrer || '' };
          window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'StoryHeal_HOST_PAGE_EXIT', payload }, '*');
        } catch(e) { /* noop */ }
      };

      // Notify controller (optional) + initial page info
      ctrl.addEventListener('load', ()=>{
        try { window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'storyheal:ui-ready', name:UI_IFRAME_NAME }, '*'); } catch(e) {}
      ...
      function createToastContainer() {
        ...
        const container = document.createElement('div');
        container.id = 'storyheal-toast-container';
        Object.assign(container.style, {
          position: 'fixed',
          bottom: '80px',
          right: '16px',
      ...
      window.addEventListener('message', (e)=>{
        const d = e.data || {};
        if(d && d.type === 'storyheal:hide') apiHide();
        if(d && d.type === 'storyheal:show') apiShow();
        if(d && d.type === 'storyheal:theme-change'){
        ...
        if(d && d.type === 'StoryHeal_WIDGET_CONFIG' && d.payload){
        ...
        if(d && d.type === 'StoryHeal_REQUEST_PAGE_INFO'){
          try { notifyPageInfoDebounced(); } catch(_){}
        }
        // Handle unread count
        if(d && d.type === 'StoryHeal_WIDGET_UNREAD' && d.payload){
          console.log('[StoryHeal SDK] Received StoryHeal_WIDGET_UNREAD, count:', d.payload.count);
          try { 
            if (state.launcher && state.launcher.setUnreadCount) {
              state.launcher.setUnreadCount(d.payload.count);
              console.log('[StoryHeal SDK] Called setUnreadCount with:', d.payload.count);
            } else {
              console.warn('[StoryHeal SDK] Launcher or setUnreadCount not available');
            }
          } catch(e){ console.error('[StoryHeal SDK] Error setting unread count:', e); }
        }
        // Handle toast message preview
        if(d && d.type === 'StoryHeal_SHOW_TOAST' && d.payload){
          console.log('[StoryHeal SDK] Received StoryHeal_SHOW_TOAST:', d.payload);
          try { showToast(d.payload); } catch(e){ console.error('[StoryHeal SDK] Error showing toast:', e); }
        }
      });
  ...
  function init(opts){
    if (__storyheal_lastInstance) return __storyheal_lastInstance;
    const script = getCurrentScript();
    const base = dirnameFromScript(script);
    console.log('[StoryHeal SDK] init', base);
    __storyheal_lastInstance = inject(base, opts||{});
    return __storyheal_lastInstance;
  }
  ...
  const queue = [];
  let booted = false;
  function flush(){ while(queue.length) { const [fn, args] = queue.shift(); try{ api[fn]?.apply(null, args);}catch(e){console.error('[StoryHeal SDK]', e);} } }
  ...
  const api = {
    ...
    show: function(){ if(!__storyheal_lastInstance) init(); return __storyheal_lastInstance?.show?.(); },
    hide: function(){ if(!__storyheal_lastInstance) init(); return __storyheal_lastInstance?.hide?.(); },
    track: function(){
      try {
        ...
        if (payload) window.frames[CTRL_IFRAME_NAME]?.postMessage({ type:'StoryHeal_TRACK_EVENT', payload }, '*');
      } catch(e) { /* noop */ }
    },
    shutdown
  };
