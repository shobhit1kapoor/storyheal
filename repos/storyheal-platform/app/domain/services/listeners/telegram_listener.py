...
from app.domain.ports import MessageNormalizer, StoryHealApiClient, SSEManager
...
class TelegramChannelListener:
    ...
    def __init__(
        ...
        storyheal_api_client: StoryHealApiClient,
        sse_manager: SSEManager,
    ) -> None:
        self._session_factory = session_factory
        self._normalizer = normalizer
        self._storyheal_api_client = storyheal_api_client
    ...
    async def _get_or_register_visitor(
        ...
        # Register or get visitor via storyheal-api
        if platform.api_key:
            try:
    ...
    async def _process_update(
        ...
        async with self._session_factory() as db:
            try:
                # 1) Get/register visitor FIRST to get the official UUID from storyheal-api
                visitor, display_name, avatar_url = await self._get_or_register_visitor(platform, msg_data)
                
                # 2) If it's an image, download from Telegram and upload to storyheal-api
                    ...
                    try:
                        ...
                        if file_info.get("ok"):
                            ...
                            if file_path:
                                ...
                                # c) Upload to storyheal-api using the real visitor.channel_id
                                ...
                                async with httpx.AsyncClient(timeout=60) as client:
                                    ...
                                    if resp.status_code == 200:
                                        ...
                                        print(f"[TELEGRAM] Failed to upload image to storyheal-api (status {resp.status_code}): {resp.text}")
                    except Exception as e:
                ...
                reply_text = await process_message(
                    msg=msg,
                    db=db,
                    storyheal_api_client=self._storyheal_api_client,
