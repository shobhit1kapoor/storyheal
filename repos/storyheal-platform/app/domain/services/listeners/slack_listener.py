...
- Uses synchronous HTTP calls to storyheal-api to avoid asyncio event loop conflicts
...
class SlackChannelListener:
    ...
    3. Sends messages to storyheal-api via synchronous HTTP (avoids asyncio conflicts)
    4. storyheal-api handles visitor registration, chat processing, and reply sending
    """
    ...
    def __init__(
        ...
        storyheal_api_client: Any,  # Not used directly anymore
    ...
    async def reload_platform_with_config(
        ...
        """Hot-reload a Slack platform with config passed directly from storyheal-api."""
        ...
        try:
                ...
                if mimetype.startswith("image/") and file_url and channel_id:
                    ...
                    try:
                        ...
                        if ul_resp.status_code in (200, 201):
                            ...
                            if uploaded_url:
                                msg_type = 2 # Image
                                # Use relative URL and let storyheal-api resolve it via settings.API_BASE_URL
                                content = uploaded_url
                                print(f"[SLACK] Image processed and uploaded: {uploaded_url}")
                        else:
                            print(f"[SLACK] Image upload to StoryHeal-API failed: {ul_resp.text}")
                    except Exception as e:
                        print(f"[SLACK] Image download/upload chain failed: {e}")

            # 3. Call storyheal-api chat endpoint using synchronous HTTP
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # Correctly extract reply from StoryHeal native response
                ...
                print(f"[SLACK] Error calling storyheal-api: {e}")
