...
These endpoints are called by storyheal-api to notify storyheal-platform about platform changes.
"""
...
class SlackPlatformConfig(BaseModel):
    """Slack platform configuration passed from storyheal-api."""
    platform_id: str
    project_id: str
...
async def reload_slack_platform(config: SlackPlatformConfig, request: Request):
    ...
    Called by storyheal-api when a Slack platform is enabled or its config is updated.
    storyheal-api passes the platform config directly to avoid cross-database queries.
    """
...
async def stop_slack_platform(platform_id: str, request: Request):
    ...
    Called by storyheal-api when a Slack platform is disabled.
    """
    slack_listener = getattr(request.app.state, "slack_listener", None)
    if not slack_listener:
        raise HTTPException(status_code=503, detail="Slack listener not initialized")
