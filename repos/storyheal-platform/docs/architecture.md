...
- storyheal-api Client + SSE Manager（HTTP + SSE）
...
flowchart LR
  ...
  subgraph StoryHeal[storyheal-platform]
    L[渠道监听器]
    N[消息标准化]
    R[路由/触发 storyheal-api]
    S[SSE 管理/聚合]
    A[平台适配器]
  end

  subgraph API[storyheal-api]
...
3) 触发：构造请求体（见 5. 接口规范），调用 storyheal-api 的 SSE 接口。
...
sequenceDiagram
  ...
  participant P as storyheal-platform
  participant API as storyheal-api
...
- Webhook 推送或轮询获取消息 → 标准化 → 调用 storyheal-api（SSE）→ 平台适配。
- 关键差异：是否支持流式消息、消息长度与速率限制、富媒体支持与回执语义。

---

### 5. 接口规范（核心 API）
基于 docs/storyheal-api.json 的定义，复用并精简关键要点：
...
### 8. 部署架构（可选）
- 推荐分层部署与水平扩展，storyheal-platform 与 storyheal-api 可独立伸缩。
...
flowchart LR
  ...
  GW --> PF[storyheal-platform]
  PF --> API[storyheal-api]
  API --> WK[WuKongIM]
  WK --> AI[AI 服务]
...
### 9. 参考与延伸
- 参考文档：docs/storyheal-platform-tech.md（核心概念与接口说明来源）
