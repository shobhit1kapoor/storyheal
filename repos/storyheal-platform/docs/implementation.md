## storyheal-platform Implementation Guide (Python/FastAPI)

本指南将 docs/architecture.md 的架构设计映射为可落地的 Python 3.11+ 实现方案，覆盖技术栈、目录结构、核心组件抽象、数据库与配置、接口与 SSE 实现、依赖与版本建议。
...
选型理由
- 全异步链路：渠道监听 → storyheal-api（SSE）→ 平台适配，需 async/await 优先（FastAPI、httpx、SQLAlchemy asyncio）。
...
app/
  ...
  domain/
    ...
    services/
      __init__.py
      dispatcher.py          # 调用 storyheal-api 客户端
...
职责：调用 storyheal-api /v1/wukongim/messages/send-stream 建立 SSE，产出事件流。
```python
...
class StoryHealApiClient(Protocol):
...
职责：
- 解析 storyheal-api 的 SSE：connected|event|error|disconnected
...
class Settings(BaseSettings):
    ...
    api_base_url: str  # storyheal-api base url
...
async def orchestrate(msg: NormalizedMessage, db: AsyncSession) -> None:
    """端到端编排：查库取平台配置（含 api_key）→ 调用 storyheal-api（SSE）→ 平台适配输出。"""
    for attempt in range(3):  # 简单重试
        try:
            ...
            frames = storyheal_api_client.send_stream(req)
...
storyheal-api 客户端（httpx）
```python
import httpx
from contextlib import asynccontextmanager

class HttpxStoryHealApiClient:
...
async def debug_pipe():
    async def event_gen():
        async for ev in sse_manager.stream_events(storyheal_api_client.send_stream(...)):
...
.env.example
```
API_BASE_URL=https://storyheal-api.example.com

DATABASE_URL=postgresql+asyncpg://user:pass@127.0.0.1:5432/storyheal
...
- 可靠性：
  - storyheal-api 超时使用 timeout_seconds，与平台适配回退策略对齐
...
async def process_message(msg: NormalizedMessage, db: AsyncSession) -> None:
    for attempt in range(3):  # 简单重试（指数退避）
        try:
            ...
            frames = storyheal_api_client.send_stream(req_body)
            events = sse_manager.stream_events(frames)
