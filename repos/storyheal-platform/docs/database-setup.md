...
This guide covers database initialization, migrations, and common operations for the storyheal-platform project.
...
```
postgresql+asyncpg://storyheal_user:storyheal_pass@127.0.0.1:5442/storyheal
```

### From Docker Network (for containerized apps)
```
postgresql+asyncpg://storyheal_user:storyheal_pass@postgres:5432/storyheal
```

**Important**: The storyheal-platform app runs on the host machine, so it uses `127.0.0.1:5442`.
...
### "role storyheal_user does not exist"
...
**Solution**: Update `.env`:
```bash
PG_DSN=postgresql+asyncpg://storyheal_user:storyheal_pass@127.0.0.1:5442/storyheal
```
