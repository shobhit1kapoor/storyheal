## storyheal-platform 技术说明（简版）

### 系统概述
`storyheal-platform` 是一个第三方平台消息聚合系统，支持企业微信、Email、WhatsApp、Telegram 等多种渠道。系统统一接入消息，调用内部 AI 能力并将结果按目标平台能力以流式或非流式方式输出。
...
- storyheal-platform
  ...
  - 消息编排：统一触发 storyheal-api 的对话/处理能力
- storyheal-api
...
时序示意（Email 场景）：
- Email → storyheal-platform（监听）→ storyheal-api（SSE）→ storyheal-platform（适配）→ 第三方平台用户

---

### 核心消息流转（以 Email 为例）
1) 消息监听：storyheal-platform 轮询或 webhook 方式监听用户配置的 SMTP 邮箱。
2) 消息触发：新邮件到达后，storyheal-platform 组装请求，并调用 storyheal-api 的流式发送接口。
3) API 处理：storyheal-api 调用 WuKongIM，AI 服务开始处理并以 SSE 事件流返回。
4) 平台适配：
   - 支持流式的平台：storyheal-platform 按事件到达实时输出。
   - 不支持流式的平台：storyheal-platform 缓存事件，待“完整结果”到齐后再一次性发送。
...
2) 调用 storyheal-api：POST `/v1/wukongim/messages/send-stream`，建立 SSE 连接。
3) 处理流事件：
