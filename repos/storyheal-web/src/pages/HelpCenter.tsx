  ...
  return (
    <main className="min-h-screen bg-[#f7f8f5] text-slate-900">
      ...
      <section className="mx-auto max-w-6xl px-5 py-10">
        <div className="mb-5 flex items-end justify-between"><div><h2 className="text-xl font-semibold">{query ? 'Search results' : 'Knowledge library'}</h2><p className="mt-1 text-sm text-slate-500">{filtered.length} published {filtered.length === 1 ? 'entry' : 'entries'} · Content Delivery API</p></div><span className="hidden rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-700 sm:block">Live from Storyblok</span></div>
