...
// 获取商店 API 地址 (通过 storyheal-api 代理)
...
const storeAuthFetch = async <T>(
  ...
  // 添加 StoryHeal API 的 token
  const storyhealToken = localStorage.getItem('storyheal-auth-token');
  if (storyhealToken) {
    (headers as Record<string, string>)['Authorization'] = `Bearer ${storyhealToken}`;
  }
...
export const storeApi = {
  ...
  installModel: async (id: string) => {
    // 调用 StoryHeal API 同步模型到本地项目 (StoryHeal API 内部会处理商店状态)
  ...
  getInstalledModels: async (): Promise<string[]> => {
    // 从本地 StoryHeal API 获取已安装模型列表 (model_id 列表)
