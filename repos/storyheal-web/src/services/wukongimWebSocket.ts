...
export type StreamFinishHandler = (clientMsgNo: string, sources?: Record<string, unknown>[]) => void;
...
export class WuKongIMWebSocketService {
    ...
    this.eventHandlers.customEvent = (event: any) => {
      ...
      try {
        ...
        if (newEventType === WS_EVENT_TYPE.STREAM_FINISH) {
          this.notifyStreamFinishHandlers(clientMsgNo, eventData.payload?.sources);
          return;
        }
  ...
  private notifyStreamFinishHandlers(clientMsgNo: string, sources?: Record<string, unknown>[]): void {
