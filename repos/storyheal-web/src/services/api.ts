/**
 * API Service Layer for StoryHeal Web
...
class APIClient {
  ...
  constructor(baseURL: string) {
    ...
    this.token = localStorage.getItem('storyheal-auth-token');
  }
  ...
  setToken(token: string | null) {
    this.token = token;
    if (token) {
      localStorage.setItem('storyheal-auth-token', token);
    } else {
      localStorage.removeItem('storyheal-auth-token');
    }
  }
