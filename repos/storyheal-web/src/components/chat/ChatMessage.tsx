...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onSuggestionClick, onSendMessage }) => {
  ...
  const sources = (Array.isArray(meta.sources) ? meta.sources : Array.isArray(typedPayload?.sources) ? typedPayload.sources : []) as Array<Record<string, unknown>>;
  ...
  if (!isOwnMessage) {
    return (
      <div className="flex flex-col items-start max-w-[85%]">
        ...
        {sources.length > 0 && <div className="ml-10 mt-2 w-[calc(100%-2.5rem)] rounded-xl border border-teal-100 bg-teal-50/70 p-3 text-xs text-slate-700 dark:border-teal-900 dark:bg-teal-950/30 dark:text-slate-200">
          <div className="mb-2 flex items-center gap-1.5 font-semibold text-teal-800 dark:text-teal-300"><BookOpen className="h-3.5 w-3.5" /> Published Storyblok sources</div>
          <div className="space-y-1.5">{sources.map((source, index) => <a key={`${String(source.story_uuid || index)}:${String(source.locale || '')}`} href={String(source.url || '#')} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-3 hover:text-teal-800 dark:hover:text-teal-200"><span className="truncate">{String(source.title || source.full_slug || `Source ${index + 1}`)}</span><ExternalLink className="h-3 w-3 shrink-0" /></a>)}</div>
        </div>}
