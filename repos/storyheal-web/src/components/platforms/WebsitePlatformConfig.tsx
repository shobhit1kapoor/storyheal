...
const getWidgetPreviewUrlWithFallback = (apiKey?: string, mode?: 'light' | 'dark', lang?: string): string => {
  ...
  const baseUrl = url || 'http://127.0.0.1:5500/storyheal-widget/dist/index.html';
...
const getWidgetScriptBase = (): string => {
  ...
  return `${getWidgetPreviewOrigin()}/storyheal-widget-sdk.js`;
};
...
const WebsitePlatformConfig: React.FC<WebsitePlatformConfigProps> = ({ platform }) => {
  ...
  const handleDownloadExample = useCallback(() => {
    ...
    const platformDisplayName = platform.display_name || platform.name || 'StoryHeal Widget';
...
<body>
    <div class="container">
        ...
        <p>${t('platforms.website.download.welcomeDesc', 'Your StoryHeal chat widget is now integrated! Click the chat button in the bottom-right corner to start a conversation.')}</p>
    ...
    <!-- StoryHeal Widget SDK - Your API Key is pre-configured -->
    ...
    link.href = url;
    link.download = `storyheal-widget-demo-${platform.id.slice(0, 8)}.html`;
