...
interface WebhookOustoryhealingProps {
  config: PlatformConfigType;
  onUpdate?: (config: PlatformConfigType) => void;
}

/**
 * Webhook oustoryhealing section component
 */
const WebhookOustoryhealing: React.FC<WebhookOustoryhealingProps> = ({ config, onUpdate }) => {
  const { t } = useTranslation();
  const [oustoryhealingUrl, setOustoryhealingUrl] = useState<string>(config?.oustoryhealingUrl || '');
  const [oustoryhealingToken, setOustoryhealingToken] = useState<string>(config?.oustoryhealingToken || '');

  const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const newUrl = e.target.value;
    setOustoryhealingUrl(newUrl);
    onUpdate?.({ ...config, oustoryhealingUrl: newUrl });
  };

  const handleTokenChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const newToken = e.target.value;
    setOustoryhealingToken(newToken);
    onUpdate?.({ ...config, oustoryhealingToken: newToken });
  };

  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md p-5 rounded-lg shadow-sm border border-gray-200/60 dark:border-gray-700/60 w-full">
      <h3 className="text-md font-semibold text-gray-700 dark:text-gray-200 mb-4">
        {t('platforms.config.webhookOustoryhealing.title', 'Webhook 推送 (发送消息)')}
      </h3>
      <div className="space-y-4">
        <div>
          <label htmlFor="oustoryhealing-url" className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">
            {t('platforms.config.webhookOustoryhealing.yourUrl', '您的接收 URL')}
          </label>
          <input
            type="url"
            id="oustoryhealing-url"
            value={oustoryhealingUrl}
            onChange={handleUrlChange}
            placeholder="https://your.system.com/receive_message"
            className="w-full text-sm p-1.5 border border-gray-300/80 dark:border-gray-600/80 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400 bg-white/90 dark:bg-gray-700/50 dark:text-gray-200"
          />
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {t('platforms.config.webhookOustoryhealing.urlHint', '当客服回复时，我们会将消息 POST 到此 URL。')}
          </p>
        </div>
        <div>
          <label htmlFor="oustoryhealing-token" className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">
            {t('platforms.config.webhookOustoryhealing.authToken', '认证 Token (可选)')}
          </label>
          <input
            type="text"
            id="oustoryhealing-token"
            value={oustoryhealingToken}
            onChange={handleTokenChange}
            placeholder={t('platforms.config.webhookOustoryhealing.tokenPlaceholder', '例如：Bearer your_secret_token')}
            className="w-full text-sm p-1.5 border border-gray-300/80 dark:border-gray-600/80 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400 bg-white/90 dark:bg-gray-700/50 dark:text-gray-200"
          />
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {t('platforms.config.webhookOustoryhealing.tokenHint', '如果您的接收 URL 需要认证，请在此处填写（例如 Authorization Header 的值）。')}
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="flex-grow overflow-y-auto p-6 space-y-6" style={{ height: 0 }}>
        ...
        {platform.type === 'custom' && (
          <>
            ...
            <WebhookOustoryhealing config={platform.config} onUpdate={(config) => onUpdate?.({ ...platform, config })} />
            <ApiAccess />
          </>
        )}
