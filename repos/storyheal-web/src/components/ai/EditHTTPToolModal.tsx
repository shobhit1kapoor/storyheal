...
import type { AiToolResponse, HttpToolConfig, HttpToolParameter, AiToolUpdateRequest } from '@/types';

interface EditHTTPToolModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const EditHTTPToolModal: React.FC<EditHTTPToolModalProps> = ({ isOpen, onClose, tool }) => {
  ...
  useEffect(() => {
    if (tool && tool.transport_type === 'http_webhook') {
      ...
      const headersArray: HeaderItem[] = Object.entries(config.headers || {}).map(([key, value]) => ({
        key,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        'success',
        t('tools.editHttpToolModal.success.title', '更新成功'),
        t('tools.editHttpToolModal.success.message', 'HTTP 工具 {{name}} 已成功更新', { name: formData.name })
      );

      await loadTools(false);
      onClose();
    } catch (error) {
      console.error('Failed to update HTTP tool:', error);
      showToast(
        'error',
        t('tools.editHttpToolModal.error.title', '更新失败'),
      ...
      <div className="bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-gray-100 dark:border-gray-800 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">
                {t('tools.editHttpToolModal.title', '编辑 HTTP 工具')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {t('tools.editHttpToolModal.subtitle', '修改外部 API 接口的配置信息')}
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="flex items-center justify-end gap-3 p-8 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 sticky bottom-0 z-10">
          ...
          >
            ...
            ) : t('tools.editHttpToolModal.save', '保存修改')}
          </button>
        </div>
      </div>
    </div>
...
export default EditHTTPToolModal;
