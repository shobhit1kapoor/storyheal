...
import { ShieldCheck, Calendar, Info, Check } from 'lucide-react';
...
const ToolStoreDetail: React.FC<ToolStoreDetailProps> = ({ 
  ...
  return (
    <StoreDetailPanel
      isOpen={isOpen}
      onClose={onClose}
      title={t('tools.store.details', '工具详情')}
      headerActions={
        <button
          ...
          <div className="flex-1 space-y-4">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 font-medium text-lg mt-1">
                {tool.author || 'StoryHeal'} <span className="text-gray-300 dark:text-gray-600 mx-2">@</span>{tool.authorHandle || 'storyheal'}
              </p>
            </div>
              ...
              {methods.map((method: ToolMethod) => {
                ...
                return (
                  <div key={method.id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm">
                    {/* Method Header */}
                    <div 
                      className={`px-5 py-3 bg-gray-50/50 dark:bg-gray-800/80 flex items-center justify-between cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors`}
